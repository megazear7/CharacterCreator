...
# check error value
		...
		$docid = $id->to_string;
		$json = qq{{"status" : "success", "msg" : "handled saveDoc request: insert", "docid" : "$docid"}};
	    } else {
		...
		if ($id->{ok} == 1) {  
		    ...
		    $json = qq{{"status" : "success", "msg" : "handled saveDoc request: update", "docid" : "$docid"}};
	        } else {
